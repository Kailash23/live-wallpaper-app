...
public class ParticleShooter {
    ...
    // The first thing we’ll do is spread out our particles, and we’ll also vary
    // the speed of each particle to give each particle fountain some more variety
    ...
    // We also have a matrix and two vectors, so we can use Android’s Matrix
    // class to do some math
    ...
    // In addParticles(), we pass in the particle system and how many particles we want
    // to add, as well as the current time for the particle system. We now have all
    // of our components in place, and we just need to add a few calls to our renderer
    // class to glue everything together.
    ...
    public void addParticles(ParticleSystem particleSystem, float currentTime,
        ...
        for (int i = 0; i < count; i++) {
            ...
            // To alter the shooting angle, we use Android’s Matrix.setRotateEulerM()
            // to create a rotation matrix that will alter the angle by a random amount of angleVariance,
            // which is in degrees.
            ...
            // We then multiply this matrix with the direction vector to
            // get a slightly rotated vector.
            multiplyMV(
                    resultVector, 0,
                    rotationMatrix, 0,
                    directionVector, 0);
            ...
            // To adjust the speed, we multiply each component of the direction
