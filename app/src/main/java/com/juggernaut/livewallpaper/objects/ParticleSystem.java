...
import com.juggernaut.livewallpaper.data.VertexArray;
import com.juggernaut.livewallpaper.programs.ParticleShaderProgram;
import com.juggernaut.livewallpaper.util.Geometry.Point;
import com.juggernaut.livewallpaper.util.Geometry.Vector;
...
public class ParticleSystem {
    ...
    public void addParticle(Point position, int color, Vector direction,
        ...
        nextParticle++;
        /*
          Each time a new particle is added, we increment nextParticle by 1, and when we reach the end,
          we start over at 0 so we can recycle the oldest particles.
         */

        if (currentParticleCount < maxParticleCount) {
            currentParticleCount++;
            /*
              We also need to keep track of how many particles need to be drawn, and we do this by
              incrementing currentParticleCount each time a new particle is added, keeping it clamped
              to the maximum.
             */
        }
        ...
        if (nextParticle == maxParticleCount) {
            /*
              Start over at the beginning, but keep currentParticleCount so that all the other
              particles still get drawn.
             */
            nextParticle = 0;
        }
        ...
        /*
          Then the color (using Android’s Color class to parse each component), then the direction vector.
          Android’s Color class returns components in a range from 0 to 255, while OpenGL expects the color
          to be from 0 to 1.
         */
        ...
        /*
          ...
          We want to copy over only the new data so that we don’t waste time copying over data that
          hasn’t changed, so we pass in the start offset for the new particle and the count.
         */
        vertexArray.updateBuffer(particles, particleOffset, TOTAL_COMPONENT_COUNT);
    }

    /**
     * Add a binding function(boiler-plate code):-binding our vertex data to the right attributes in
     * the shader program and taking care to respect the same ordering as the one we used in addParticle().
     */
    ...
    /**
     * We now have a particle system in place. This system will let us add particles up to a certain
     * limit, recycle old particles, and efficiently locate the particles next to each other in memory
     */
    public void draw() {
